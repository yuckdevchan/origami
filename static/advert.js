...
function game_is_over_main() {
    ...
    if (game_mode.slice(0, 6) == 'paper2') {
        ...
        if (game_with_modes) {
            $(share + '<div class="button play" style="display: none" onclick="game_start(true);">PLAY AGAIN</div><div class="button mode" style="display: none" onclick="document.location.assign(document.location.protocol + \'//\' + document.location.host);">CHANGE GAME MODE</div>').appendTo('#game_over').hide()
        } else {
            $(share + '<div class="button play" style="display: none" onclick="game_start(true);">PLAY AGAIN</div>').appendTo('#game_over').hide()
        }
    } else {
        if (game_with_modes) {
            $('<div class="button share" style="display: none; width: 160px;" onclick="window.open(\'https://www.facebook.com/sharer/sharer.php?u=https://paper-io.com/sharescore/' + my_sco_re.toFixed(2) + '/' + playtime_m + ':' + playtime_s + '\',\'sharer\',\'toolbar=0,status=0,width=580,height=325\');">SHARE</div><div class="button play" style="display: none" onclick="game_start(true);">PLAY AGAIN</div><div class="button mode" style="display: none" onclick="document.location.href = document.location.href;">CHANGE GAME MODE</div>').appendTo('#game_over').hide()
        } else {
            $('<div class="button share" style="display: none; width: 160px;" onclick="window.open(\'https://www.facebook.com/sharer/sharer.php?u=https://paper-io.com/sharescore/' + my_sco_re.toFixed(2) + '/' + playtime_m + ':' + playtime_s + '\',\'sharer\',\'toolbar=0,status=0,width=580,height=325\');">SHARE</div><div class="button play" style="display: none" onclick="game_start(true);">PLAY AGAIN</div>').appendTo('#game_over').hide()
        }
    }
    if (game_with_modes) {
        $('#bottom').css({
